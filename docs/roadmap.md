...
- それ以外（席バーコード注文・残数表示など）はモック（固定データ＋ボタン遷移）でOK。動画で動いて見えれば十分。
...
> UIは他メンバーが実装し main にマージ済み → `shimizu/feature` に取り込み済み。店舗側・学生側の主要画面は揃っている。残るは席バーコードによる注文方法のみ。

- [x] 店舗側: 食材の登録画面（撮影/アップロード→機能①→食材リスト）。
- [x] 店舗側: 食材の確認・修正画面（価格・在庫点数(packCount)・アレルギーを編集して食材リストを確定 → 機能②へ）。
- [x] 店舗側: メニュー候補の表示・公開画面（候補3件から1つ選んで「公開」）。
- [x] 学生側: 今日のメニュー・残数一覧（注文ボタンあり）
...
- [ ] スマホ幅のレイアウト・配色（現行ロゴを使用）。
- [ ] 具体シナリオのダミーデータ確定 → 実写と画面を編集して紹介動画（約1:15）に。
- [ ] プレゼン資料に動画を組み込む。
...
2. 学生側のモック画面（今日のメニュー・残数 → 席バーコード注文 → 注文確認）。
