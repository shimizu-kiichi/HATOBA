...
- `allergens`: その食材のアレルゲン（28品目から該当。機能①で付与済み）
...
各リスト食材には在庫点数(packCount)とアレルゲン(allergens)も付く。「在庫全体で何食作れるか」はシステムが
packCount と usageRatio から計算するので、AIはこの計算をしない（点数の多い食材を主役に選ぶ判断には使ってよい）。
各食材の allergens は、そのメニューの allergens を決めるときに必ず反映する（下記参照）。
...
- allergens: そのメニューのアレルゲン。次の2つを合わせ、重複は1つにまとめて、次の28品目から選ぶ（該当がなければ空配列）。
  1. 使ったリスト食材それぞれに付いている allergens を必ず全て含める
  2. recipe で足す調味料・追加食材（例: 醤油→小麦・大豆）から推定されるアレルゲンも加える
